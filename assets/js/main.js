(function () {
    ...
    window.addEventListener('load', () => {
        setInterval(() => {
            const wheels = document.querySelectorAll('.randomize-wheel-right ul');
            for (let wheel of wheels) {
                if (wheel.children.length === 0)
                    continue;

                var firstChild = wheel.children[0];
                wheel.classList.remove('move-up');
                wheel.appendChild(firstChild);
                setTimeout(() => {
                    wheel.classList.add('move-up');
                }, 0);
            }
        }, 4000);
    });
